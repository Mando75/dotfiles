# The prompt
PROMPT='$(_user_host)$(_python_venv)%{$fg[magenta]%}%c $(git_prompt_info)%{$reset_color%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}λ '

# Prompt with SHA
# PROMPT='$(_user_host)$(_python_venv)%{$fg[magenta]%}%c $(git_prompt_info)%{$reset_color%}$(git_prompt_short_sha)%{$fg[magenta]%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}➜ '
